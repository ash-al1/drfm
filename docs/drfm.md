...
Knowledge on DRFM systems is very sparsely found on the internet. Using the well known Phillip E. Pace book on developing DRFM modules for EW we could build a very basic foundation on how our drone is supposed to act at a high level, the kind of situations expected in deployment and realism on implementation. This is not a comprehensive list of information although sets the foundation on ideas we managed to pull from the book. Expect to see this content grow over time, as implementation continues to evolve and increase in complexity the closer to real-world we get.
