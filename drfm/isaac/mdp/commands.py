...
class WaypointCommand(CommandTerm):
    ...
    def _is_clear(self, pos: torch.Tensor) -> torch.Tensor:
        ...
        for center, half in self.cfg.obstacles:
            c = torch.tensor(center, device=pos.device)
            h = torch.tensor(half, device=pos.device) + margin
...
class WaypointCommandCfg(CommandTermCfg):
    ...
    # ((cx, cy, cz), (hx, hy, hz)) axis-aligned box obstacles; center + half-extents in local frame
    obstacles: tuple = (
        ((6.0, 8.0, 2.5), (1.0, 1.0, 2.5)),
        ((13.5, -8.0, 2.0), (1.0, 0.5, 2.0)),
        ((22.0, 8.0, 3.5), (1.5, 1.5, 3.5)),
